...
int main() {
    try {
        auto on_initialize = [](engene::EnGene& app) {
            ...
            unsigned char greyPixel[] = { 128, 128, 128, 255 };
            auto glossMap = texture::Texture::Make(1, 1, greyPixel);
            ...
            scene::graph()->getNodeByName("CameraNode")->payload().get<component::TransformComponent>()->getTransform()->translate(0,0,5);
        ...
        auto on_fixed_update = [](double dt) {
            if (auto sphere = scene::graph()->getNodeByName("Sphere")) {
                sphere->payload().get<component::TransformComponent>()->getTransform()->rotate(0.5f * (float)dt, 0, 1, 0);
            }
        };
